TYPE
	...
	END_STRUCT;
	McASMConType : STRUCT (*Settings for the convoys*)
		MaximumCount : UINT; (*Maximum count of convoys in the assembly*)
	END_STRUCT;
	...
	McCfgAsmType : STRUCT (*Main data type corresponding to McCfgTypeEnum mcCFG_ASM*)
		...
		Convoys : McASMConType; (*Settings for the convoys*)
	...
	END_STRUCT;
	McSSTStatTransUsrCtrlReCtrlEnum :
		( (*Setting of whether the user can control state transitions from Disabled to a different state*)
		mcSSTSTUCRC_INACT := 0, (*Inactive - State transitions from Disabled are done automatically*)
		mcSSTSTUCRC_ACT := 1 (*Active - State transitions from Disabled are controlled by the user*)
		);
	McSSTStatTransType : STRUCT (*Parameter settings for state transitions*)
		UserControlledRecontrol : McSSTStatTransUsrCtrlReCtrlEnum; (*Setting of whether the user can control state transitions from Disabled to a different state*)
	END_STRUCT;
